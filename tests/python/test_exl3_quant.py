...
class TestLoraDequantizeDispatchContract(unittest.TestCase):
    """The LoRA-only dispatch must honour the bnb transpose contract.
    ...
    def setUp(self):
        # Import the opt-in LoRA dispatch from the kernels module. The public
        # fast_dequantize remains the bitsandbytes implementation.
        from unsloth.kernels.utils import _dequantize_for_lora
        self.dequantize = _dequantize_for_lora
    ...
    def test_placeholder_transposed_gives_in_out(self):
        ...
        w_t = self.dequantize(W, qs, transpose = True)
